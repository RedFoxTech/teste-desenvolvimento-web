...
const UpdatePokemon = () => {
    ...
    const id = localStorage.getItem('pokemon_id');
    
    useEffect(() => {
        async function getElements(){
           await api.get(`http://localhost:3333/pokemons/${id}`).then(response => {
    ...
    const [name1, setName] = useState<string>(String(pokemons?.name));
  /*  const [pokedex, set] = useState<string>(String(pokemons?.name));
    const [name, setName] = useState<string>(String(pokemons?.name));
    const [name, setName] = useState<string>(String(pokemons?.name));
    const [name, setName] = useState<string>(String(pokemons?.name));*/
    return (
                    ...
                    <div>
                        <Form.Row className="formG">
                            <Col xs={9}>
                                ...
                                <Form.Control name="name" value={name1} as="input" onChange={handleInputChange} placeholder="Digite o nome do Pokemon" required />
                            </Col>
                            <Col>
