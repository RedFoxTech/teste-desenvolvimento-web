...
class PokemonController {
  ...
  public async index (req:Request, res:Response):Promise<Response> {
    try {
      if (req.query.id) {
        const pokemon = await Pokemon.findOne({ where: { id: req.query.id } })
        return res.status(200).json(pokemon)
      }
      if (req.query.pokedex_number) {
        const pokemon = await Pokemon.findOne({ where: { pokedexNumber: req.query.pokedex_number } })
        return res.status(200).json(pokemon)
      }
      if (req.query.name) {
        const pokemon = await Pokemon.findOne({ where: { name: req.query.name } })
        return res.status(200).json(pokemon)
      }

      let pokemons:Pokemon[] = []
      if (req.query.type) {
        pokemons = await Pokemon.findAll({ where: { [Op.or]: [{ type1: req.query.type }, { type2: req.query.type }] } })
      } else {
  ...
  public async update (req:Request, res:Response):Promise<Response> {
    try {
      ...
      if (fileLocation && fileKey) {
        Object.assign(pokemon, req.body, { imgName: fileKey, imgUrl: fileLocation })
  ...
  private calculateTotalStat (atk:number, def:number, sta:number) {
    return atk + def + sta
  }
}
