...
<p align="justify">
Projeto desenvolvido para o teste de desenvolvimento web da RedFox, onde foi dada maior ênfase ao backend, visto que é minha atual área de atuação no momento. </p>
<p align="justify">
Como o teste possibilita uma flexibilidade sobre a forma na qual o objetivo será atingido, optei por utilizar na modelagem do banco de dados MySQL os seguintes campos: name, pokedex_number, generation, evolution, familyID, type_1, type_2, weather_1, weather_2, stat_total, atk, def, sta, legendary, cp@39, cp@40, os demais campos não foram utilizados.  </p>
<p align="justify">
Os campos que possuíam maior relacionamento entre os pokémons eram type_1 e weather_1, por isso optei por criar na tabela de pokemons os campos type_1, type_2, weather_1 e weather_2 como chaves estrangeiras das tabelas types e weather, sendo que cada Pokémon possuía um types e weather e as tabelas types e weather possuem vários Pokémons. </p>
...
```
<p align="justify">
A API possui controle de sessão através de um token JWT, na qual possibilita a divisão das rotas em públicas e privadas, sendo que as rotas publicas podem ser acessadas sem o token enquanto as privadas não podem ser acessadas sem um token válido.  </p>
...
|--|--|--|--|--|
| POST | /user | Cadastra um usuário | name, email e password | Antes de cadastrar um usuário é verificado se o e-mail informado já foi cadastrado. |
| PUT | /user | Atualiza os dados do usuário | Se existir oldPassword os campos password e confirPassword passam a ser obrigatórios.  | Verifica se a senha confere com a salva no banco e se o e-mail informado já não foi cadastrado. |
...
| GET | /types | Traz todos os types cadastrados | |Verifica se existe algum type cadastrado e traz todos os types cadastrados. |
...
| GET | /pokemon | Traz todos os pokémons cadastrados| |Busca todos os pokémons através do filtro de ativos ou inativos e ordena pelo name.|
| GET | /pokemon/:id | Traz todos os pokémons cadastrados| |Verifica se o pokémons existem e traz os dados deste Pokémon.|
| PUT | /pokemon/:id | Atualiza os dados de um pokémon| | Verifica se o pokémons existem e se já não existe outro Pokémon com o mesmo nome antes de atualizar os dados. |
...
Funções:
<blockquote><strong>handleLoginClick:</strong> Faz uma requisição à API para criar uma seção.</blockquote>
...
| Rota |Função | Descrição |
|--|--|--|
| /updatepokemon | Rota para atualizar um Pokémon.  | Para ser acessada basta clicar no Pokémon desejado na lista. |
