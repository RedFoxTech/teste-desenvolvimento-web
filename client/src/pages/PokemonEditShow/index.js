...
function FormAddPokemonFields({
  ...
  useEffect(() => {
    loadSelects();
    if (location.state) {
      for (let key in location.state.pokemon) {
        setFieldValue(key, location.state.pokemon[key]);
      }
    }
  }, []);
  ...
  return (
    <Form>
      ...
      >
        <div></div>
        <h1 className="mb-5 mt-5">Edit/Show Pokemon</h1>
        <div className="mr-5">
          <CustomButton
            label="Back"
            onClick={() => {
      ...
      <div className="d-flex justify-content-center">
        <Form style={{ width: '80%' }}>
          ...
          <div style={{ width: 500, margin: 'auto' }}>
            <Row className="mt-2 d-flex justify-content-around mx-auto mb-5">
              <CustomButton
                label="Delete"
                onClick={async () => {
                  await api.delete(`pokemons/${location.state.pokemon.id}`);
                  history.push('/');
                }}
                variant="danger"
                type="button"
